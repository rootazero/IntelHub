...
//!
//! Covers the design spec's T1 list plus an atomic-write check and the boot
//! path (`start()`: disk → warm cache → flusher):
...
//! `dirty_flush_atomic`, `start_loads_disk_cache_and_flushes_dirty`.
...
#[tokio::test]
async fn cache_load_persist_roundtrip() {
    ...
    cache.persist_to(&file).await.unwrap();

    let loaded = EnrichmentCache::load_from(&file).await.unwrap();
...
/// The boot path end-to-end: a cache file on disk becomes a warm in-process
/// cache, and a later mutation is persisted by the spawned flusher alone (no
/// test-driven `persist_to` call).
#[tokio::test]
async fn start_loads_disk_cache_and_flushes_dirty() {
    let dir = tempdir_unique("adsbdb-start");
    let file = dir.join("adsbdb-cache.json");

    // 1. Pre-seed the on-disk cache — the "previous process" warm set.
    let mut seed = EnrichmentCache::new();
    seed.aircraft.insert(
        "4ca9b1".to_string(),
        CachedAircraft { at: now_ms(), data: Some(aircraft_fixture()) },
    );
    seed.routes.insert(
    ...
    seed.persist_to(&file).await.expect("seed persist");

    // Upstream answers only for the *new* key; anything else 404s.
    let upstream = MockServer::start().await;
    Mock::given(method("GET"))
        .and(path("/v0/aircraft/abcdef"))
    ...
    let svc = Arc::new(test_service(&upstream.uri()));
    svc.start(file.clone(), Duration::from_millis(50)).await;

    // 2. disk → warm cache: the seeded keys are answered with zero upstream
    //    calls (a miss here would hit the mock and bump the counter).
    let res = svc.aircraft("4ca9b1").await;
    assert_eq!(res.status(), StatusCode::OK);
    let body = axum::body::to_bytes(res.into_body(), usize::MAX).await.unwrap();
    let body: serde_json::Value = serde_json::from_slice(&body).unwrap();
    assert_eq!(body["found"], true);
    assert_eq!(body["registration"], "EI-DCL");
    // Callsign is normalised to uppercase before the cache lookup.
    let res = svc.route("ual123").await;
    assert_eq!(res.status(), StatusCode::OK);
    let body = axum::body::to_bytes(res.into_body(), usize::MAX).await.unwrap();
    let body: serde_json::Value = serde_json::from_slice(&body).unwrap();
    assert_eq!(body["airline"], "United Airlines");
    assert_eq!(svc.upstream_calls().await, 0, "seeded disk cache must be served warm");

    // 3. A real fetch marks the cache dirty...
    let res = svc.aircraft("abcdef").await;
    assert_eq!(res.status(), StatusCode::OK);
    assert_eq!(svc.upstream_calls().await, 1);

    // 4. ...and the spawned flusher writes it out with nobody calling
    //    persist_to. Poll (50ms cadence, up to 2s) for the new key on disk.
    let mut flushed: Option<serde_json::Value> = None;
    for _ in 0..40 {
        if let Ok(s) = tokio::fs::read_to_string(&file).await {
            if s.contains("abcdef") && s.contains("4ca9b1") {
                flushed = serde_json::from_str(&s).ok();
                break;
            }
        }
        tokio::time::sleep(Duration::from_millis(50)).await;
    }
    let flushed = flushed.expect("flusher must persist the dirty cache within ~2s");
    assert_eq!(
        flushed["aircraft"].as_object().unwrap().len(),
        2,
        "both the warm entry and the freshly fetched one must persist"
    );
    assert_eq!(flushed["routes"].as_object().unwrap().len(), 1);
}
