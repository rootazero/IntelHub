...
check("OSINT Framework bridge collectors present (31/31)",
      len(present) == 31, f"present={','.join(present)},missing={','.join(set(osint_bridge)-set(present))}")
...
check("OSINT bridge /api/v1/health surfaces 31 collectors (structured)",
      len(ob_collectors) == 31 and ob_names == set(osint_bridge),
...
check("OSINT bridge collectors carry cadence_hours (31/31)",
      len(ob_with_cadence) == 31, f"missing-cadence={set(osint_bridge)-{c['name'] for c in ob_with_cadence}}")
...
# So minimum ok = 28/31 (allow 3 to fail or be absent).
check("OSINT bridge structured ok >= 28/31 (GFW+OpenCorp+OpenSanctions may shelve)", len(ob_ok) >= 28,
