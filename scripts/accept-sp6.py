...
check("OSINT Framework bridge collectors present (22/22)",
      len(present) == 22, f"present={','.join(present)},missing={','.join(set(osint_bridge)-set(present))}")
...
check("OSINT bridge /api/v1/health surfaces 22 collectors (structured)",
      len(ob_collectors) == 22 and ob_names == set(osint_bridge),
...
check("OSINT bridge collectors carry cadence_hours (22/22)",
      len(ob_with_cadence) == 22, f"missing-cadence={set(osint_bridge)-{c['name'] for c in ob_with_cadence}}")
...
# So minimum ok = 19/22 (allow 3 to fail or be absent).
check("OSINT bridge structured ok >= 19/22 (GFW+OpenCorp+OpenSanctions may shelve)", len(ob_ok) >= 19,
