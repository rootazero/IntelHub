...
check("OSINT Framework bridge collectors present (25/25)",
      len(present) == 25, f"present={','.join(present)},missing={','.join(set(osint_bridge)-set(present))}")
...
check("OSINT bridge /api/v1/health surfaces 25 collectors (structured)",
      len(ob_collectors) == 25 and ob_names == set(osint_bridge),
...
check("OSINT bridge collectors carry cadence_hours (25/25)",
      len(ob_with_cadence) == 25, f"missing-cadence={set(osint_bridge)-{c['name'] for c in ob_with_cadence}}")
...
# So minimum ok = 22/25 (allow 3 to fail or be absent).
check("OSINT bridge structured ok >= 22/25 (GFW+OpenCorp+OpenSanctions may shelve)", len(ob_ok) >= 22,
