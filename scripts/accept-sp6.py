...
check("OSINT Framework bridge collectors present (28/28)",
      len(present) == 28, f"present={','.join(present)},missing={','.join(set(osint_bridge)-set(present))}")
...
check("OSINT bridge /api/v1/health surfaces 28 collectors (structured)",
      len(ob_collectors) == 28 and ob_names == set(osint_bridge),
...
check("OSINT bridge collectors carry cadence_hours (28/28)",
      len(ob_with_cadence) == 28, f"missing-cadence={set(osint_bridge)-{c['name'] for c in ob_with_cadence}}")
...
# So minimum ok = 25/28 (allow 3 to fail or be absent).
check("OSINT bridge structured ok >= 25/28 (GFW+OpenCorp+OpenSanctions may shelve)", len(ob_ok) >= 25,
