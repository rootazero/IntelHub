...
- **scene-panel CSS selector drift（vendor vs intelhub）**（2026-09-19 GEV P10 教训）：vendor scene-panel DOM id 是 `#scene-panel`，intelhub HUD 子组件用 `data-testid="hud-scene-panel"`。两者**不**冲突因为 intelhub 不挂载 vendor shell（自有 React HUD 框架），但 P11 同步时 vendor 若重命名为统一 selector 需要 P10 测试 fixture 跟进。Mirror rule: 每次 vendor 同步（console/gev-engine/）后跑 `console/src/globe-hud/__tests__/hud-scene-panel.test.tsx` 验证 testid 仍可达
...
- **dismissSearch Escape recursion（315-specific，410 未复现）**（2026-09-19 GEV P10 教训）：T4 probe 在 315 上抓到 46 pageerrors，源头是 vendor search-overlay dismiss 触发 Escape 再次调用 `dismissSearch` 形成的无限递归；410 baseline 未复现（P9 ledger 验收时无此 pageerror），属 315 浏览器实例特性。**不阻塞 push**。P11 修复方向：vendor `dismissSearch` 加 `if (searchOverlay.classList.contains('dismissed')) return;` 早返回守卫
